...
class GPTFp8Weights(object):
    ...
    def __init__(self,
                 ...
                 int8_mode=0,
                 fp8_mode=0,
                 weights_data_type=typing.Union[str, np.float32]):
        ...
        self.scale = []
        # Transformer blocks
        self.w.extend([torch.zeros(global_hidden_units, dtype=torch.bfloat16)] * layer_num)  # self_layernorm_gamma
        self.w.extend([torch.zeros(global_hidden_units, dtype=torch.bfloat16)] * layer_num)  # self_layernorm_beta
        self.w.extend([torch.zeros(global_hidden_units, local_hidden_units * 3, dtype=torch.bfloat16)] *
                      layer_num)  # self_kernel
        self.w.extend([torch.zeros(local_hidden_units * 3, dtype=torch.bfloat16)] * layer_num)  # self_bias
        self.w.extend([torch.zeros(local_hidden_units, global_hidden_units, dtype=torch.bfloat16)] *
                      layer_num)  # self_output_kernel
        self.w.extend([torch.zeros(global_hidden_units, dtype=torch.bfloat16)] * layer_num)  # self_output_bias
        self.w.extend([torch.zeros(global_hidden_units, dtype=torch.bfloat16)] * layer_num)  # ffn_layernorm_gamma
        self.w.extend([torch.zeros(global_hidden_units, dtype=torch.bfloat16)] * layer_num)  # ffn_layernorm_beta
        self.w.extend([torch.zeros(global_hidden_units, local_inter_size, dtype=torch.bfloat16)] *
                      layer_num)  # ffn_kernel1
        self.w.extend([torch.zeros(local_inter_size, dtype=torch.bfloat16)] * layer_num)  # ffn_bias1
        self.w.extend([torch.zeros(local_inter_size, global_hidden_units, dtype=torch.bfloat16)] *
                      layer_num)  # ffn_kernel2
        self.w.extend([torch.zeros(global_hidden_units, dtype=torch.bfloat16)] * layer_num)  # ffn_bias2
        # After Transformer blocks
        if self.has_post_decoder_layernorm:
            self.w.append(torch.zeros(global_hidden_units, dtype=torch.bfloat16))  # layernorm_gamma
            self.w.append(torch.zeros(global_hidden_units, dtype=torch.bfloat16))  # layernorm_beta
        self.w.append(torch.zeros(max_seq_len, global_hidden_units, dtype=torch.bfloat16))  # position_encoding_table
        self.w.append(torch.zeros(vocab_size, global_hidden_units, dtype=torch.bfloat16))  # embedding_table
...
pipeline_para_rank and i < self.layers_per_device * (pipeline_para_rank + 1)
        ...
        w.extend([
            torch.from_numpy(
                np.fromfile(ckpt_path + "/model.layers.{}.input_layernorm.weight.bin".format(i),
                            dtype=self.weights_data_type)) if is_load(i) else torch.empty(0)
            for i in range(self.layer_num)
        ])
        w.extend([
            torch.from_numpy(
                np.fromfile(ckpt_path + "/model.layers.{}.input_layernorm.bias.bin".format(i),
                            dtype=self.weights_data_type)) if is_load(i) else torch.empty(0)
            for i in range(self.layer_num)
        ])
        w.extend([
            torch.from_numpy(
                np.fromfile(ckpt_path +
                            "/model.layers.{}.attention.query_key_value.weight.{}.bin".format(i, tensor_para_rank),
                            dtype=self.weights_data_type)) if is_load(i) else torch.empty(0)
            for i in range(self.layer_num)
        ])
        w.extend([
            torch.from_numpy(
                np.fromfile(ckpt_path +
                            "/model.layers.{}.attention.query_key_value.bias.{}.bin".format(i, tensor_para_rank),
                            dtype=self.weights_data_type)) if is_load(i) else torch.empty(0)
            for i in range(self.layer_num)
        ])
        w.extend([
            torch.from_numpy(
                np.fromfile(ckpt_path + "/model.layers.{}.attention.dense.weight.{}.bin".format(i, tensor_para_rank),
                            dtype=self.weights_data_type)) if is_load(i) else torch.empty(0)
            for i in range(self.layer_num)
        ])
        w.extend([
            torch.from_numpy(
                np.fromfile(ckpt_path + "/model.layers.{}.attention.dense.bias.bin".format(i),
                            dtype=self.weights_data_type)) if is_load(i) else torch.empty(0)
            for i in range(self.layer_num)
        ])
        w.extend([
            torch.from_numpy(
                np.fromfile(ckpt_path + "/model.layers.{}.post_attention_layernorm.weight.bin".format(i),
                            dtype=self.weights_data_type)) if is_load(i) else torch.empty(0)
            for i in range(self.layer_num)
        ])
        w.extend([
            torch.from_numpy(
                np.fromfile(ckpt_path + "/model.layers.{}.post_attention_layernorm.bias.bin".format(i),
                            dtype=self.weights_data_type)) if is_load(i) else torch.empty(0)
            for i in range(self.layer_num)
        ])
        w.extend([
            torch.from_numpy(
                np.fromfile(ckpt_path + "/model.layers.{}.mlp.dense_h_to_4h.weight.{}.bin".format(i, tensor_para_rank),
                            dtype=self.weights_data_type)) if is_load(i) else torch.empty(0)
            for i in range(self.layer_num)
        ])
        w.extend([
            torch.from_numpy(
                np.fromfile(ckpt_path + "/model.layers.{}.mlp.dense_h_to_4h.bias.{}.bin".format(i, tensor_para_rank),
                            dtype=self.weights_data_type)) if is_load(i) else torch.empty(0)
            for i in range(self.layer_num)
        ])
        w.extend([
            torch.from_numpy(
                np.fromfile(ckpt_path + "/model.layers.{}.mlp.dense_4h_to_h.weight.{}.bin".format(i, tensor_para_rank),
                            dtype=self.weights_data_type)) if is_load(i) else torch.empty(0)
            for i in range(self.layer_num)
        ])
        w.extend([
            torch.from_numpy(
                np.fromfile(ckpt_path + "/model.layers.{}.mlp.dense_4h_to_h.bias.bin".format(i),
                            dtype=self.weights_data_type)) if is_load(i) else torch.empty(0)
            for i in range(self.layer_num)
        ])
        ...
        if self.has_post_decoder_layernorm:
            w.append(
                torch.from_numpy(
                    np.fromfile(ckpt_path + "/model.final_layernorm.weight.bin", dtype=self.weights_data_type)))
            w.append(
                torch.from_numpy(
                    np.fromfile(ckpt_path + "/model.final_layernorm.bias.bin", dtype=self.weights_data_type)))

        wpe = torch.from_numpy(np.fromfile(ckpt_path + "/model.wpe.bin",
                                           dtype=self.weights_data_type)).reshape(-1, self.global_hidden_units)
        ...
        if os.path.isfile(ckpt_path + "/model.lm_head.weight.bin"):
            w.append(
                torch.from_numpy(np.fromfile(ckpt_path + "/model.lm_head.weight.bin", dtype=self.weights_data_type)))
        else:
            w.append(torch.from_numpy(np.fromfile(ckpt_path + "/model.wte.bin", dtype=self.weights_data_type)))
...
class GPTFp8(nn.Module):
    ...
    def __init__(
            ...
            ckpt_path,
            layernorm_eps=1e-6,
            layernorm_type="pre_layernorm",  # gpt_variant_params
            activation_type="Gelu",
            has_post_decoder_layernorm=True,  # gpt variant params
            int8_mode=0,
            fp8_mode=1,
        ...
        # Prepare weights
        # if self.ckpt_path != '':
        self.weights = GPTFp8Weights(head_num,
                                     size_per_head,
                                     layer_num,
                                     vocab_size,
                                     max_seq_len,
        ...
        self.model = torch.classes.FasterTransformer.GptFp8Op(
            self.head_num, self.size_per_head, 4 * self.head_num * self.size_per_head, self.layer_num, self.vocab_size,
            self.max_seq_len, self.start_id, self.end_id, self.tensor_para_size, self.pipeline_para_size,
            self.layernorm_eps, self.layernorm_type, self.activation_type, self.ckpt_path,
            self.has_post_decoder_layernorm, self.weights.w)
    ...
    def from_args(cls, args, config_reader):
        ...
        return cls(
            ...
            pipeline_para_size=(args.pipeline_para_size or
                                config_reader.getint("ft_instance_hyperparameter", "pipeline_para_size")),
            int8_mode=(args.int8_mode if args.int8_mode is not None else config_reader.getint(
                "ft_instance_hyperparameter", "int8_mode")),
    ...
    def from_args(cls, args, config_reader):
        return cls(
            ...
            beam_search_diversity_rate=(args.beam_search_diversity_rate or config_reader.getfloat(
                "ft_instance_hyperparameter", "beam_search_diversity_rate")),
            ...
            repetition_penalty=(args.repetition_penalty or
                                config_reader.getfloat("ft_instance_hyperparameter", "repetition_penalty")),
        )
